...
import { getStatusBlockSurface, workloadPercentToTone } from '@/lib/ui/status-block-tone';

export function WorkloadIndicator({ member, className }: { member: WorkloadMember; className?: string }) {
  const tone = workloadPercentToTone(member.percent);
  return (
    <div className={cn('space-y-2 p-3', getStatusBlockSurface(tone, { urgentGlow: tone === 'danger' }), className)}>
