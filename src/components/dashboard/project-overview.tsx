...
export function ProjectOverview({ projects }: { projects: ProjectRowMock[] }) {
  return (
    <SectionCard title="Projets en cours" description="Clients et internes — progression fictive.">
      <ul className="space-y-4">
        {projects.map((p) => (
          <li key={p.id} className="rounded-xl border border-primary/20 bg-gradient-to-br from-card to-surface-secondary p-3">
            ...
            {p.blocker ? <p className="mt-2 text-xs text-[#FF6A2A]">Blocage : {p.blocker}</p> : null}
          </li>
        ))}
      </ul>
    </SectionCard>
