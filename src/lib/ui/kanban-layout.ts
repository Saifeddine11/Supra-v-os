...
export const KANBAN_COLUMN_WIDTH_CLASS = 'w-[280px] shrink-0 flex-none';

/** Hauteur fixe des colonnes (viewport − chrome page). */
export const KANBAN_COLUMN_HEIGHT_CLASS = 'h-[min(calc(100dvh-11.5rem),760px)]';

/** Carte du board kanban (tasks / videos). */
export const KANBAN_BOARD_OUTER_CLASS =
  'w-full overflow-hidden rounded-2xl border border-border/60 bg-muted/15 p-1.5 dark:bg-muted/10 md:p-2';
...
export const KANBAN_COLUMNS_ROW_CLASS = 'flex w-max min-w-full items-start gap-3 px-0.5 md:px-1';
