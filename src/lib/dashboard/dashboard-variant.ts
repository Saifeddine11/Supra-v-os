 ...
 */
export type DashboardVariant = 'admin' | 'manager' | 'supervisor' | 'commercial' | 'finance' | 'individual';

export function getDashboardVariant(role: UserRole | null, isDepartmentSupervisor = false): DashboardVariant {
  if (!role) return 'individual';
  if (role === 'admin') return 'admin';
  if (role === 'project_manager') return 'manager';
  if (role === 'department_supervisor' || isDepartmentSupervisor) return 'supervisor';
...
export function dashboardScopeFromRole(role: UserRole | null, isDepartmentSupervisor = false): DashboardScope {
  if (role === 'admin') return 'full';
  if (role === 'project_manager') return 'operations';
  if (role === 'department_supervisor' || isDepartmentSupervisor) return 'operations';
  if (role === 'finance') return 'finance';
  if (role === 'commercial') return 'commercial';
  return 'individual';
}
