...
export function VideoKanbanColumn({
  ...
  return (
    <div
      className={cn(
        'flex w-[min(100vw-2rem,320px)] shrink-0 flex-col rounded-2xl border bg-card/95 shadow-sm dark:bg-card/90 md:w-[300px]',
    ...
    >
      ...
      <div
        ref={setNodeRef}
        className={cn(
          'flex max-h-[min(70vh,720px)] min-h-[148px] flex-1 flex-col gap-2.5 overflow-y-auto rounded-b-2xl p-2.5 transition-colors',
          dragEnabled && droppable && isOver && 'bg-primary/[0.07] dark:bg-primary/[0.1]',
        )}
      >
        {items.length === 0 ? (
          <div
